...
- `src/components/` — `Hero.astro` (includes a Don Giussani quote above the date badge; date badge is styled like the event time-badges — uppercase text flanked by thin lines, no filled pill; no scroll-hint button; optional `rsvpHref` prop renders an RSVP CTA button at the bottom), `PhotoStrip.astro` (reused 3x), `EventStep.astro` (reused for "La Santa Messa" and "La Festa" via a `variant` prop; photo sits beside the details card, side by side — `photoPosition` prop, `"left"` for La Messa, `"right"` for La Festa, stacking below 640px; card has no description paragraph anymore — just venue name/address/photo/maps link — and stretches to match the photo's height via `align-items: stretch`, centered on mobile; optional `rsvpHref` prop renders an "RSVP" titled block with confirmation text + CTA button below the card, used on both La Messa and La Festa), `Gifts.astro`, `Footer.astro`
- `src/pages/index.astro` — assembles all sections in order; defines a shared `rsvpHref` constant (the Google Form URL for RSVP) passed to `Hero` and both `EventStep` instances
